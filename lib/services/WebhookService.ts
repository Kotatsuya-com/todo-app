...
export interface WebhookServiceInterface {
  getUserWebhooks(userId: string): Promise<ServiceResult<UserWebhooksResult>>
  createUserWebhook(params: CreateWebhookParams): Promise<ServiceResult<WebhookResult>>
  deactivateWebhook(webhookId: string, userId: string): Promise<ServiceResult<void>>
}

export class WebhookService implements WebhookServiceInterface {
  constructor(private slackRepo: SlackRepositoryInterface) {}
  ...
  async getUserWebhooks(userId: string): Promise<ServiceResult<UserWebhooksResult>> {
    try {
      ...
      const webhooksResult = await this.slackRepo.findWebhooksByUserId(userId)
      
      if (!webhooksResult.success) {
        return {
          success: false,
          error: webhooksResult.error || 'Failed to fetch webhooks',
          statusCode: 500
        }
      }
  ...
  async createUserWebhook(params: CreateWebhookParams): Promise<ServiceResult<WebhookResult>> {
    ...
    try {
      // Slack接続の存在確認
      const connectionResult = await this.slackRepo.findConnectionById(slackConnectionId)
      ...
      // 既存のWebhookがあるかチェック
      const existingWebhookResult = await this.slackRepo.findWebhookByConnectionId(userId, slackConnectionId)
      
      if (existingWebhookResult.success && existingWebhookResult.data) {
        ...
        const updateResult = await this.slackRepo.updateWebhook(
      ...
      // 新しいWebhookを作成
      const createResult = await this.slackRepo.createWebhook({
        user_id: userId,
  ...
  async deactivateWebhook(webhookId: string, userId: string): Promise<ServiceResult<void>> {
    try {
      ...
      // Webhookを非有効化
      const updateResult = await this.slackRepo.updateWebhook(webhookId, { is_active: false })
